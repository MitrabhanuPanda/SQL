...
select s.streetaddr,s.starttime,s.endtime from servicepoint as s
...
select sum(quantity) as Tquan from save_to_shopping_cart as d
inner join product as p
on d.pid=p.pid
where sid=8;

# 4. Query the name and address of orders delivered on 2017-02-17?
select name,streetaddr,city from address as a
inner join deliver_to as dd
on a.addrid=dd.addrid
...
update address
set name='', contactPhoneNumber=''
where province='Quebec' and city='Montreal';


# 9.Create view of all products whose price above average price.?
create view price_above_avg_price as
select pid,name,price
from product
where price>(select avg(price) from product);

select * from price_above_avg_price;


# 10.Create view of all products sales in 2016.?
create view sales_product_2016 as
select p.pid,p.name,p.price 
from product as p
inner join orderitem as o
on p.pid=o.pid
inner join contain as c
on o.itemid=c.itemid
inner join payment as a
on c.ordernumber=a.ordernumber
where paytime>'2016-12-31';

select * from sales_product_2016;
select distinct(pid),name,price from sales_product_2016;
